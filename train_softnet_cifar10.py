...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        output = model(data)
        loss = Cosine_Similarity_Loss()(output,target)
        loss.backward()
        optimizer.step()
...
def eval_train(model, device, train_loader):
    ...
    with torch.no_grad():
        for data, target in train_loader:
            data, target = data.to(device), target.to(device)
            output = model(data)
            train_loss += Cosine_Similarity_Loss()(output,target).item()
            correct += get_correct_num(output,target)
...
def eval_test(model, device, test_loader):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            data, target = data.to(device), target.to(device)
            output = model(data)
            test_loss += Cosine_Similarity_Loss()(output,target).item()
            correct += get_correct_num(output,target)
